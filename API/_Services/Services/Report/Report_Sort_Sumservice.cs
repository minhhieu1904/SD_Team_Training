...
{
    public class Report_Sort_Sumservice : IReport_Sort_Sumservice
    {
        private readonly IRepositoryAccessor _reposioryAccessor;
        private readonly IWebHostEnvironment _webHostEnvironment;
        private DBContext _dbContext;

        public Report_Sort_Sumservice(IRepositoryAccessor reposioryAccessor, IWebHostEnvironment webHostEnvironment, DBContext dbContext)
        {
        ...
        public async Task<PaginationUtility<Report_Sort_SumParam>> GetData(PaginationParam pagination, SortSumDTO param, bool isPaging = true)
        {
            var data = await _dbContext.Report_Sort_SumParam.FromSqlRaw
            ...
            return PaginationUtility<Report_Sort_SumParam>.Create(data, pagination.PageNumber, pagination.PageSize, isPaging);
        }
        ...
        public async Task<byte[]> ExportExcelDetails(SortSumDeltailDTOParam param)
        {

            List<SortSumDeltailDTO> data = await GetDataDetails(param);  
            MemoryStream stream = new MemoryStream();
            if (data.Any())
              {
                var path = Path.Combine(_webHostEnvironment.ContentRootPath, "Resources\\Template\\Report\\4.2Report_Sort_Sum\\Report_Sort_Sums_Details.xlsx");
        ...
        private async Task<List<SortSumDeltailDTO>> GetDataDetails(SortSumDeltailDTOParam param)
        {
            ...
            var result = await _reposioryAccessor.MS_QR_Label.FindAll(predicate)
                        ...
                        .GroupJoin(_reposioryAccessor.MS_QR_Sort.FindAll().AsNoTracking(),
                        x => x.T1.QRCodeID,
                        y => y.QRCodeID,
                        (x, y) => new { T1 = x.T1, T2 = x.T2, T3 = y })
                        .SelectMany(x => x.T3.DefaultIfEmpty(), (x, y) => new { T1 = x.T1, T2 = x.T2, T3 = y })
                        .Select(x => new SortSumDeltailDTO()
                        {
                            IsScanSort = x.T1.QRCodeID != null ? "Y" : "N",
                            CrDay = x.T3.CrDay,
