...
namespace API._Services.Services.Transaction
{
    public class S_SearchForOrderDataServices : I_SearchForOrderDataServices
    {
        ...
        {
            var pred = PredicateBuilder.New<MS_QR_Order>(x => x.CycleFlag == "N" && x.Manuf == "N");
            ...
            var data = await _repositoryAccessor.MS_QR_Order.FindAll(pred).Select(x => new SearchForOrderDataDTO
            {
