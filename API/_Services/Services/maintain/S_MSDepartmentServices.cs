...
namespace API._Services.Services
{
    public class SMSDepartmentServices : IMSDepartmentServices
    {
        ...
        {
            return await _repositoryAccessor.MS_Department.FirstOrDefaultAsync(x => x.Manuf == manuf && x.ParNo == parNo);
        }

        public async Task<PaginationUtility<MS_Department>> GetPagingData(PaginationParam param, string parNo, string parName)
        {
            var data = await _repositoryAccessor.MS_Department.FindAll().ToListAsync();
        ...
        {
            var data = await _repositoryAccessor.MS_Department.FirstOrDefaultAsync(x => x.Manuf == model.Manuf && x.ParNo == model.ParNo);
            if (data == null)
                return new OperationResult(false);
