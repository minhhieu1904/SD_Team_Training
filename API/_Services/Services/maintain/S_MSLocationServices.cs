...
namespace API._Services.Services
{
    public class SMSLocationServices : IMSLocationServices
    {
        ...
        public async Task<OperationResult> AddNew(MS_Location model)
        {
            ...
            var originalItem = await _repositoryAccessor.MS_Location.FirstOrDefaultAsync(x => x.Manuf.Trim() == "N" && x.StoreH.Trim() == model.StoreH.Trim());
        ...
        {
            var data = await _repositoryAccessor.MS_Location.FindAll().ToListAsync();
            return data;
        }
        ...
        {
            var data = await _repositoryAccessor.MS_Location.FirstOrDefaultAsync(x => x.Manuf == manuf && x.StoreH == storeH);
            return data;
        }
        ...
        public async Task<OperationResult> UpdateLocation(MS_Location model)
        {
            ...
            var originalItem = await _repositoryAccessor.MS_Location.FirstOrDefaultAsync(x => x.Manuf == model.Manuf && x.StoreH == model.StoreH);
